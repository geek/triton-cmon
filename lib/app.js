...
App.prototype.fetchVm = function _fetchVm(arg, lobj, datacb, cb) {
    var self = this;
    var done = false;
    var query = { limit: lobj.limit, marker: lobj.marker, state: 'active' };
    var queryString = mod_qs.stringify(query);
    var vms = [];
    self.vmapi.get('/vms?' + queryString, function (err, req, res, objs) {
        if (err) {
            self.log.error({ fetchVmError: err }, 'fetchVm failed');
            // TODO: Decide if this should be handled gracefully...
        }

        var obj_arr = JSON.parse(objs.body);
        if (obj_arr.length === 0) {
            done = true;
            self.log.debug({ processed: self.offset }, 'no more results');
        }
...
App.prototype.vmapiMarker = function _vmapiMarker(obj) {
    mod_assert.uuid(obj.uuid);
    var marker = { uuid: obj.uuid };
    return (marker);
};
