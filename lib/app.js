...
function App(opts) {
    ...
    bootstrapper.on('bootstrapped', function () {
        self.log.debug('bootstrapper emitted bootstrapped');
        bootstrapper.unpipe(updater);
    ...
    var serverOpts = {
        ...
        httpsServerOptions: self.config.tls
    };

    if (self.config.tls) {
        var tls = self.config.tls;
        mod_assert.string(tls.key, 'config.tls.key');
        mod_assert.string(tls.cert, 'config.tls.cert');
        mod_assert.string(tls.ca, 'config.tls.ca');

        tls.key = mod_fs.readFileSync(tls.key);
        tls.cert = mod_fs.readFileSync(tls.cert);
        tls.ca = mod_fs.readFileSync(tls.ca);
    ...
    server.use(lib_common.authorizationParser);
