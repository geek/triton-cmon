...
function App(opts) {
    ...
    if (self.config.useTls) {
        ...
        tlsOpts.key = mod_fs.readFileSync(TLS_KEY);
        tlsOpts.cert = mod_fs.readFileSync(TLS_CERT);
        mod_assert.string(tlsOpts.key, 'config.tls.key');
        mod_assert.string(tlsOpts.cert, 'config.tls.cert');
