...
function apiGetMetrics(req, res, next) {
    ...
    if (!cache.vms[vm_uuid]) {
        req.log.trace({ vm_cache: cache, z_uuid: vm_uuid }, 'VM not in cache');
        res.header('content-type', 'text/plain');
        res.statusCode = 404;
        next();
    } else {
        mod_assert.object(cache.vms[vm_uuid], 'vm cache obj');
        var vmCacheItem = cache.vms[vm_uuid];

        mod_assert.uuid(vmCacheItem.server_uuid, 'server_uuid');
        var server_uuid = vmCacheItem.server_uuid;

        mod_assert.object(cache.admin_ips[server_uuid], 'admin ip cache obj');
        var serverCacheItem = cache.admin_ips[server_uuid];

        mod_assert.string(serverCacheItem.admin_ip, 'admin_ip');
