...
function apiGetMetrics(req, res, next) {
    ...
    mod_assert.isObject(cache, 'cache object');
    mod_assert.isObject(cache.vms, 'cache.vms object');
    mod_assert.isObject(cache.nics, 'cache.nics object');

    var server_uuid = cache.vms[vm_uuid];
    mod_assert.string(server_uuid, 'server_uuid');

    var admin_nic = cache.nics[server_uuid];
    mod_assert.string(admin_nic, 'admin_nic');

    var cmon_agent_url = 'http://' + admin_nic + ':9163';
